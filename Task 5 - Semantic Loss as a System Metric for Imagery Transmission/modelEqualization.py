...
class ChannelEqualization(nn.Module):
    def __init__(self, channel_type='AWGN', snr=20, equalization_flag = 1):
        #if channel_type not in ['AWGN', 'Rayleigh','Rician']:
        #    raise Exception('Unknown type of channel')
        super(ChannelEqualization, self).__init__()
        self.channel_type = channel_type
        self.snr = snr
        self.equalization_flag = equalization_flag
    ...
    def forward(self, z_hat):
        ...
        if self.channel_type == 'Rayleigh':
            hc = torch.randn(2, device = z_hat.device)/torch.sqrt(torch.tensor(2.0, device=z_hat.device))
            if self.equalization_flag==1:
                h = hc[0] + 1j * hc[1]
                hinv = 1 / h
                hinvc = torch.tensor([hinv.real, hinv.imag], device=z_hat.device)

                n = noise.size(1) // 2
                left = noise[:, :n]
                right = noise[:, n:]

                noise[:,:n] = hinvc[0] * right - hinvc[1] * left
                noise[:,n:] = hinvc[0] * left + hinvc[1] * right
                ...
                z_hat[:,:n] = hc[0] * right - hc[1] * left
                z_hat[:,n:] = hc[0] * left + hc[1] * right

        if self.channel_type == 'Rician':
            K = torch.tensor(100.0, device=z_hat.device)
            # NLOS component ~ CN(0,1)
            h_nlos = torch.randn(2, device=z_hat.device) / torch.sqrt(torch.tensor(2.0, device=z_hat.device))
            phi = 2 * torch.pi * torch.rand(1, device=z_hat.device)
            h_los = torch.cat([torch.cos(phi), torch.sin(phi)])
            
            # Rician fading coefficient
            hc = torch.sqrt(K/(K+1.0)) * h_los + torch.sqrt(1.0/(K+1.0)) * h_nlos
            if self.equalization_flag==1:
                h = hc[0] + 1j * hc[1]
                hinv = 1 / h
                hinvc = torch.tensor([hinv.real, hinv.imag], device=z_hat.device)
...
class DeepJSCC(nn.Module):
    def __init__(self, c, channel_type='AWGN', snr=None, equalization_flag = 1):
        super(DeepJSCC, self).__init__()
        self.encoder = _Encoder(c=c)
        if snr is not None:
            self.channel = ChannelEqualization(channel_type, snr, equalization_flag)
    ...
    def change_channel(self, channel_type='AWGN', snr=None, equalization_flag = 1):
        if snr is None:
            self.channel = None
        else:
            self.channel = ChannelEqualization(channel_type, snr, equalization_flag)
